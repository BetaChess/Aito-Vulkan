...
namespace aito
{
		...
		void* pUserData)
	{
		...
		if (messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT)
		{
			...
			AITO_ERROR("Validation layer: {}", pCallbackData->pMessage);
		}
		else if (messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT)
		{
			//printWColor("\nvalidation layer WARNING: " << pCallbackData->pMessage, WARNCOL);
			AITO_WARN("Validation layer: {}", pCallbackData->pMessage);
		}
		else
		{
			//printWColor("validation layer INFO: " << pCallbackData->pMessage, INFOCOL);
			AITO_TRACE("validation layer: {}", pCallbackData->pMessage);
		}
		...
		: window_{ window }
	{
		...
		AITO_INFO("Starting Instance Creation");
		createInstance();
		AITO_INFO("Setting up debug messenger");
		setupDebugMessenger();
		AITO_INFO("Creating surface");
		createSurface();
